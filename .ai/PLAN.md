...
**依赖链（每节点读上一节点的产物）**：`.ai/outputs/node1.txt=42` → `node2=142` → `node3=284` → `node4=200`。
...
对齐用 **`.ai/TASK.md` 顶部 `NODE: k`** 与 **`.ai/RESULT.md` 结论区 `NODE: k`**。末节点为 **4**。
...
### 节点 3 / 目标 3：上一步 ×2
- 前提：`.ai/outputs/node2.txt` 存在。
- 派发（`NODE: 3`）：读 node2 的 142，×2，产出 `.ai/outputs/node3.txt` 含 `result=284`。
- 成功标准：RESULT `NODE: 3`；`node3.txt` 含 `result=284`；退出码 0。
- [ ] 完成

### 节点 4 / 目标 4：上一步 −84（收尾）
- 前提：`.ai/outputs/node3.txt` 存在。
- 派发（`NODE: 4`）：读 node3 的 284，−84，产出 `.ai/outputs/node4.txt` 含 `result=200`。
